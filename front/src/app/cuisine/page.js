...
const CuisinePage = () => {
	const { events, fetchEvents, filterEventsByCategory } = useEventStore();
	...
	useEffect(() => {
		const fetchAndFilterEvents = async () => {
			await fetchEvents();
			filterEventsByCategory('cuisine');
		};

		fetchAndFilterEvents();
	}, [fetchEvents, filterEventsByCategory]);
				...
				<div className='container flex-1 px-4 py-8 pl-24 mx-auto'>
					...
					>
						{currentItems.map((event) => (
							...
							>
								<img
									src={getFullImageUrl(event.attributes.picture?.data?.attributes?.url)}
